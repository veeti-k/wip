...
import { useAddWorkoutExerciseMutation } from "../AddWorkoutExerciseSlide/useAddWorkoutExerciseMutation";
...
export function CreateModelExerciseSlide() {
	...
	const addExerciseMutation = useAddWorkoutExerciseMutation();
	...
	function onSubmit(values: CreateExerciseFormType) {
		return mutation
			...
			.then((createdExerciseId) =>
				addExerciseMutation
					.mutateAsync({ workoutId, modelExerciseId: createdExerciseId })
					.then(() => {
						closeModal();
						form.reset();
						scrollToLastExercise();
	...
	useEffect(() => {
		form.setValue("name", addWorkoutExerciseSearchQuery);
	}, [addWorkoutExerciseSearchQuery, form]);
